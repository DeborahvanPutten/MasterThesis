...
N_treatments <- 2  # 
N_dams_per_sire <- 3
N_offspring_per_dam_per_treatment <- 15  # 15 offspring per dam per treatment
N_offspring_per_dam <- N_treatments * N_offspring_per_dam_per_treatment  # 30 offspring per dam
N <- N_sires * N_dams_per_sire * N_offspring_per_dam

#treatment groups
treatment <- rep(1:2, length.out = N)
N_treatment <- length(unique(treatment))

#effect of treatment on mass and baseline a_mass
a_mass <- 400 
b_mass_effects <- c(0, -0.75) #treatment specific effect

# effect of treatment on mortality
b_mor_effects <- c(0, 0.5) #treatment effect on mortality
...
# Matrices for sire effects on mass and mortality across 2 treatments
sire_effect_mass <- matrix(rnorm(N_sires * 2, 0, sigma_sire), N_sires, 2)
sire_effect_mor <- matrix(rnorm(N_sires * 2, 0, sigma_sire), N_sires, 2)

# random dam effects on pupal mass and mortality
...
for (i in 1:N) {
  ...
  #expected pupal mass and mortality probability based on the fixed and rancom effects
...
m_genetic <- ulam(
  alist(
    ...
    # fixed priors
    cholesky_factor_corr[2]:rho ~ lkj_corr_cholesky(1), # correlation structure, 1 meaning no favoring for high or low correlations
    ...
    # fixed priors
    cholesky_factor_corr[N_treatment]: rho_sire ~ lkj_corr_cholesky(1),  # Correlation between sire effects across traits
